...
if __name__ == '__main__':
	num_sensors = 1 #one more than there is a record for
	centres = gen_centres(1, 10,10, 0)
	all_data = [sklearn.externals.joblib.load('data/ind_writer')]
	all_data = [i[0:100] for i in all_data]
	merged = sorted(list(itertools.chain(*all_data)))
	streamer = data_generator(merged, 1, 5)
	for index, new_data in enumerate(streamer):
		fig, ax = plt.subplots(1)
		update(fig, ax, centres, new_data, 5)
		fig.savefig('plot/explore%d.png'%index, format = 'png')
